# Paper Claim Audit — FRESH (2026-04-20)
...
**CONDITIONAL PASS.** All material quantitative claims are traceable to authoritative result files and reproduce. One internal inconsistency persists: the Introduction pairs `think@512=56.9%` with `avg_tokens=477`, but these come from *different* seed runs (56.9% is seed-42; 477 is seed-11 whose accuracy is 65.2%). The paper acknowledges the cross-seed mixing in appendix §G Table 16 (lines 822-828) but the intro presents them as a single figure. One minor baseline value (8B GSM8K n=200 Stage-3 baseline, reported as 90.0% vs. evidence showing TOWN=89.0% / IRIS=93.0%) is 1.0pp imprecise. The remaining ~40 numerical claims all reproduce.
