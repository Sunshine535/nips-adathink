...
\begin{equation}
  F_L(b^*) \;=\;
    \frac{\bar{\alpha}_{\mathrm{nt}} - \alpha_t(b^*)}
         {\alpha_c(b^*) - \alpha_t(b^*)}.
  \label{eq:crossover-exact}
\end{equation}
When $\alpha_t(b^*) \approx 0$ (strong truncation regime) and
...
$F_L(b^*) \alpha_c(b^*) + (1 - F_L(b^*)) \alpha_t(b^*) = \bar{\alpha}_{\mathrm{nt}}$,
which rearranges to Eq.~\eqref{eq:crossover-exact}.
The simplified heuristic follows by setting $\alpha_t = 0$ and treating
...
$F_L(b^*) \approx \bar{\alpha}_{\mathrm{nt}} / \alpha_c \approx 0.955/0.963 = 0.992$.
...
$\alpha \ge 0.963 - \sqrt{\ln(2/0.05)/(2 \cdot 475)} = 0.901$.
...
Under the heuristic (Eq.~\ref{eq:crossover}),
$b^*(M) \approx F_{L_M}^{-1}(\bar{\alpha}_{\mathrm{nt}}/\alpha_c)$;
since $F_{L_M}^{-1}$ shifts right with $M$:
...
for moderate $b_a$).
Split-budget generation is advantageous at matched total budget when
the first term (truncation recovery) exceeds the second
(additional completion benefit).
\end{proposition}
