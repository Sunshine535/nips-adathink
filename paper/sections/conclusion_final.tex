...
We presented the \couplingtax{}: a systematic empirical and theoretical study showing that coupling reasoning and answering under a shared token budget imposes a dramatic accuracy penalty below a task- and model-specific \emph{crossover budget}---${\sim}$2048 tokens on GSM8K, with the crossover shifting to even higher budgets on harder benchmarks (MATH-500)---with the tax scaling inversely with model size.
Our failure-mode analysis reveals the causal mechanism: 98.6\% of thinking responses are truncated at budget 256, vs.\ only 11.2\% in non-thinking mode.
We scope this finding to Qwen-family open reasoning models with explicit \texttt{<think>} mode under fixed output-token budgets.
...
The natural-stop signal provides a free confidence oracle with 99.0\% positive predictive value, motivating \method{} (\fullmethod{}), a training-free inference framework that allocates \emph{separate} budgets for reasoning and answering via \splitbudget{}.
\method{} achieves \textbf{94.0\%} accuracy on GSM8K at 235 avg tokens (+5.0\,pp over nothink@256, pilot $n{=}200$)---the only method to improve beyond the nothink plateau.
...
Our theoretical framework makes testable predictions: the crossover budget scales with the 97th percentile of chain-length distributions (Proposition~\ref{prop:crossover}), the tax grows with model size via stochastic dominance of chain lengths (Proposition~\ref{prop:inverse-scaling}), and the budget multiplier for thinking to become cost-effective is at least $4\times$ on GSM8K (Corollary~\ref{cor:multiplier}), with harder benchmarks requiring even larger multipliers.
As frontier models grow and chains lengthen, these results suggest the thinking tax will worsen---not improve---making adaptive routing between reasoning modes essential.
...
Our study focuses on structured reasoning tasks (mathematical, logical, symbolic); the tax may differ for tasks where CoT is structurally essential, such as multi-hop reasoning requiring intermediate retrieval, code generation requiring iterative debugging, or agentic workflows with tool use, or stochastic decoding regimes where best-of-$K$/self-consistency can amortize the truncation cost.
Experiments on BIG-Bench Hard (Appendix~\ref{app:bbh}) extend the finding beyond mathematics, but comprehensive validation on natural language understanding or open-ended generation remains future work.
