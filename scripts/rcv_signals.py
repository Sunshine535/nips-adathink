...
def answer_validity_score(text: str, benchmark: str) -> Dict[str, float]:
    """Score answer validity based on format and parsing signals."""
    if not text or not text.strip():
        return {"valid": 0.0, "has_number": 0.0, "has_boxed": 0.0, "length": 0}
...
def stage0_acceptance_features(
    ...
    validity = answer_validity_score(raw_text, "gsm8k")
...
def prefix_recoverability_features(
    ...
    Compares strict vs soft extraction to estimate extraction margin.
    """
    strict_valid = strict_pred is not None and strict_pred != ""
    soft_valid = soft_pred is not None and soft_pred != ""
    ...
    has_therefore = 1.0 if any(w in prefix.lower() for w in ["therefore", "thus", "so the answer", "final answer"]) else 0.0
...
def extractor_margin(
    ...
    if strict_pred is not None and strict_pred != "":
        score += 0.3
    if soft_pred is not None and soft_pred != "":
        score += 0.2
...
def compute_rcv_decision(
    ...
    if stage0_features["natural_stop"] > 0.5:
        accept_score = (
            stage0_features["answer_valid"] * 0.5
            + (1.0 - stage0_features["pred_is_none"]) * 0.3
            + stage0_features["parse_source_boxed"] * 0.2
        )
